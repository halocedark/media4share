...
rebindEvents = () =>
{
	...
	getJoinedGroups().then(response =>
	{
		...
		{
			html += `<li>
						<a href="views/shared-with-me-in-group.ejs" data-role="ACCORDION_SHARING_GROUP" data-groupid="${ v.groupId }">${ v.groupName }</a>
					</li>`;
		});
		// Add html
...
getPage(APP_DIR_NAME+'views/statistics.ejs').then(response =>
{
	MAIN_CONTENT_CONTAINER.html(response);
	// Re assign events
	rebindEvents();
